...
//TODO: Review these queires below as they were generated by AI

export const GET_LEARNING_BITS_BY_JOURNEY = gql`
  query getLearningBitsByJourney($learningJourneyId: uuid!) {
    learningBits(
      where: { learningJourneyId: { _eq: $learningJourneyId } }
      order_by: { position: asc }
    ) {
      id
      title
...
export const GET_LEARNING_BITS_BY_USER_AND_JOURNEY = gql`
  query getLearningBitsByUserAndJourney(
    $userId: String!
    $learningJourneyId: uuid!
  ) {
    learningBits(
      where: {
        userId: { _eq: $userId }
        learningJourneyId: { _eq: $learningJourneyId }
      }
      order_by: { position: asc }
    ) {
      id
